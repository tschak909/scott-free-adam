...
doneit:				printf ("The game is now over.\n");
			...
			case 65:
			{
				...
				//if(Options&YOUARE)
					...
					printf ("I've stored ");
				#endif
				printf("%u treasures.\nOn a scale of 0 to 100, that rates %u.\n",n,(n*100)/GameHeader.Treasures);
			...
			case 85:
				...
				printf ("%s\n",NounText);
				printf ("%c[%um",27,37); // ??? and why this one does the same ?
				break;
			case 86:
				printf ("\n");
				break;
...
void main(int argc, char *argv[])
{
	...
	do
	{
		printf ("\nWhich adventure do you wish to play ?  ");
		LineInput(buf);
		ng=atoi(buf);
		sprintf(fname, "adv%u.dat", ng);
		f=fopen(fname,"r");
	}
	while (f == NULL);
...
Coleco ADAM CP/M port by Thomas Cherryhomes\n\n");
